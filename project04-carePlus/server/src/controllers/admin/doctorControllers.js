import { getDoctorsCountQuery, getPopularDoctorsListQuery, getDoctorProfileQuery } from "../../queries/admin/doctorQueries.js";
...
async function getDoctorsCount(_, res) {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, data: response, message: "Get total doctors count successfully!" });
  } catch (error) {
...
async function getPopularDoctorsList(req, res) {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, data: response, message: "Get popular doctors successfully!" });
  } catch (error) {
