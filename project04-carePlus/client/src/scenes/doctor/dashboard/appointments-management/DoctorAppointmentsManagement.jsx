...
function DoctorAppointmentsManagement() {
  ...
  const {mutate: acceptAppointment} = useMutation({
    mutationFn: (appointmentId) => axiosInstance.post('/doctor/appointments/accept', appointmentId),
    onSuccess: () => {
  ...
  const {mutate: cancelAppointment} = useMutation({
    mutationFn: (appointmentId) => axiosInstance.post('/doctor/appointments/cancel', appointmentId),
    onSuccess: () => {
  ...
  const COLUMNS = columnDefinition({ handleAccept, handleCancel, isCancelling, isAccepting });
  const columns = useMemo(() => COLUMNS, []);
  const memoizeData = useMemo(() => appointmentsList || [], [appointmentsList]);

  const { getTableProps, getTableBodyProps, headerGroups, rows, prepareRow, state: {globalFilter}, setGlobalFilter } =
    useTable({
      columns,
      data: memoizeData,
    }, useGlobalFilter);
