...
function TotalAnalytics() {
  const { data: appointmentsCount, isLoading: loadingAppointments, isError: appointmentsError } = useQuery({
  ...
  const { data: doctorsCount, isLoading: loadingDoctors, isError: doctorsError } = useQuery({
  ...
  const { data: patientsCount, isLoading: loadingPatients, isError: patientsError } = useQuery({
  ...
  if (appointmentsError || doctorsError || patientsError) {
    return <ErrorResponse error={appointmentsError || doctorsError || patientsError} />;
  }
    ...
    <>
      <div className="flex justify-between gap-4">
        ...
        <div className="w-1/3 bg-blue-100 p-6 rounded-lg shadow-md text-center">
          ...
          <p className="mt-4 text-4xl font-bold text-blue-900">{appointmentsCount}</p>
        </div>
        ...
        <div className="w-1/3 bg-green-100 p-6 rounded-lg shadow-md text-center">
          ...
          <p className="mt-4 text-4xl font-bold text-green-900">{doctorsCount}</p>
        </div>

        {/* Total Patients */}
        <div className="w-1/3 bg-yellow-100 p-6 rounded-lg shadow-md text-center">
          <h3 className="text-xl font-semibold text-yellow-700">Total Patients</h3>
          <p className="mt-4 text-4xl font-bold text-yellow-900">{patientsCount}</p>
        </div>
      </div>
    </>
  );
