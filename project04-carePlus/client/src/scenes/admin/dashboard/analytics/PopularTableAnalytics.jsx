...
const PopularTableAnalytics = () => {
  ...
  const { data: popularDoctors, isLoading, isError, error } = useQuery({
    queryKey:    ["popular-doctors", rowLimit],
    queryFn: async () => {
        const response = await axiosInstance.get(`/admin/doctors/popular?limit=${rowLimit}`);
  ...
  const getPopularityShade = (popularity) => {
    const maxPopularity = Math.max(...memoizedData.map((doc) => doc.appointment_count));
      ...
      <table {...getTableProps()} className="w-full border-collapse">
        <thead>
          {headerGroups.map((headerGroup, i) => (
            <tr key={i} {...headerGroup.getHeaderGroupProps()} className="bg-blue-100 text-gray-700">
        ...
        <tbody {...getTableBodyProps()}>
          {rows.map((row) => {
            ...
            return (
              <tr key={row.id} {...row.getRowProps()} style={{ color: rowStyle }}>
                {row.cells.map((cell) => (
                  <td key={cell.column.id} {...cell.getCellProps()} className="border p-2">
                    {cell.render("Cell")}
                  </td>
                ))}
              </tr>
