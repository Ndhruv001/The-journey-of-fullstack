    ...
    useEffect(() => {
        ...
        async function loadData() {
            try {
                ...
                updateUser(user);
                getMessages(user.username);  // Fetch messages after user data is available
            } catch (error) {
    ...
    return (
        <>
            <Navbar
                ...
                buttonName="Logout"
                content={<h1 className="font-bold text-xl">Welcome {userData.username}</h1>}
            />
                ...
                <div>
                    ...
                    {loading ? (
                        ...
                        messages.map((message, index) => (
                            <MessageBox key={index} message={message.messages} createdAt={message.createdAt} />
                        ))
                    ) : (
                        <h2 className="text-xl font-semibold mb-2">No messages to display.</h2>
                    )}
                    </div>
                </div>
            </div>
